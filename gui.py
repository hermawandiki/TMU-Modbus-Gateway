...
class MainWindow(QWidget):
    ...
    def create_main_page(self):
        ...
        if not logo_bd.isNull():
            label_logo_bd.setPixmap(logo_bd.scaledToHeight(45, Qt.TransformationMode.SmoothTransformation))

        label_logo_tmu = QLabel()
        logo_tmu = QPixmap("assets/logoTMU.png")
        if not logo_tmu.isNull():
            label_logo_tmu.setPixmap(logo_tmu.scaledToHeight(35, Qt.TransformationMode.SmoothTransformation))
        ...
        top_bar_layout.addStretch()
        top_bar_layout.addWidget(btn_exit, alignment=Qt.AlignmentFlag.AlignVCenter)
        ...
        for row, item in enumerate(visible_items):
            ...
            layout_row.addWidget(label_status)    
            layout_row.addWidget(label_number)
            
            grid_layout.addWidget(frame_row, row, 0)
            
            self.label_val[name] = label_number
            self.label_stat[name] = label_status  
            
        grid_layout.setRowStretch(len(visible_items), 1)
    ...
    def run_auto_scroll(self):
        ...
        if now_pos >= max_pos: self.dir_scroll = -1
        elif now_pos <= 0: self.dir_scroll = 1
    ...
    def hitung_dan_tampilkan_status(self, name, val):
        ...
        if "Temperature" in name:
            if val >= 80.0:
                text, color = "TRIP", "#D32F2F"
            elif val >= 70.0:
                text, color = "ALARM", "#F57C00"
            elif val >= 30.0:
                text, color = "NORMAL", "#388E3C"
            elif val < 30.0:
                text, color = "ABNORMAL", "#F57C00"
    ...
    def refresh_data_from_file(self):
        ...
        try:
            ...
            self.label_timestamp.setText(f"Last Update: {time.strftime('%H:%M:%S', time.localtime())}")
    ...
    def format_value_with_unit(self, unit, value):
        ...
        if not unit:
            return f'<span style="font-size:24px; color:#000080; font-weight:700;">{text_val}</span>'

        return (
            f'<span style="font-size:24px; color:#000080; font-weight:700;">{text_val}</span>'
